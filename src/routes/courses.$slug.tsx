    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="glass-strong rounded-3xl max-w-md w-full p-8 border border-border/80 shadow-2xl relative text-center flex flex-col items-center">
        ...
        <button
          onClick={() => {
            onClose();
            // ব্যাচ ৩ এ রিডাইরেক্ট করা হচ্ছে
...
function Batch1Landing({ course }: { course: any }) {
  ...
  return (
    <div className="pt-24 sm:pt-32 pb-12 sm:pb-16 overflow-x-hidden">
      {/* ব্যাচ ১ এর "Enroll Batch 01" বাটনে চাপলে এই পপ-আপ আসবে */}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start mb-16">
            ...
            <div className="glass-strong rounded-3xl border border-border/70 shadow-lg relative overflow-hidden backdrop-blur-md">
              <div className="p-5 sm:p-8 space-y-5">
                ...
                <div className="space-y-4 text-sm sm:text-base text-foreground/80 leading-[1.7] font-bangla border-t border-border/40 pt-5">
                  ...
                  <p>এই সমস্যার সমাধান হিসেবে আমরা আয়োজন করেছি <strong>১৫ দিনের Free Video Editing Course</strong> যেখানে প্রতিদিন Step-by-Step শেখানো হবে।</p>
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-28">
            <div className="glass-strong rounded-3xl p-6 sm:p-8 border border-border/60 shadow-xl overflow-hidden backdrop-blur-md">
              
              <div onClick={() => previewVideoId && setActiveVideo(previewVideoId)} className={`relative aspect-video w-full rounded-2xl overflow-hidden border border-border/40 group mb-6 ${previewVideoId ? "cursor-pointer" : ""}`}>
                <img src={course.thumb || ""} alt="Batch 1 Poster" className="w-full h-full object-cover pointer-events-none select-none" />
                {previewVideoId && (
          ...
          <div className="glass-strong rounded-3xl border border-border/80 p-6 sm:p-10 font-bangla shadow-lg">
            ...
            <div className="space-y-4 max-w-2xl mx-auto">
              {[
                ...
                <div key={i} className="flex items-center gap-5 p-5 rounded-2xl glass border border-border/60 shadow-sm">
...
// =========================================================================
// ২. মাস্টারক্লাস ল্যান্ডিং পেজ: BATCH 2 & BATCH 3 (ডাটাবেজ যুক্ত)
// =========================================================================
...
function TabOverview({ courseSlug }: { courseSlug: string }) {
  ...
  const badPoints = ["ঘণ্টার পর ঘণ্টা এলোমেলো ইউটিউব টিউটোরিয়ালে বিভ্রান্ত ও দিকহারা থাকা", "সাউন্ড ডিজাইন ও কালার সাইকোলজি ছাড়া সাধারণ কাট পেস্ট এডিট", "মার্কেটপ্লেসে অল্প টাকায় কাজের জন্য বিড করে বারবার রিজেক্ট হওয়া"];
  const goodPoints = ["সরাসরি প্র্যাকটিক্যাল প্রজেক্ট ও সিনেমাটিক স্টোরিটেলিং পদ্ধতি আয়ত্ত করা", "উন্নত সাউন্ড ডিজাইন, নিখুঁত কালার গ্রেডিং ও হাই রিটেনশন মোশন অ্যানিমেশন", "আন্তর্জাতিক মানের প্রফেশনাল পোর্টফোলিও ও সরাসরি ক্লায়েন্ট ডিল ক্লোজিং দক্ষতা"];

  const { items: dynCards, addItem: addCard, removeItem: removeCard, isAdmin } = useDynamicCmsList(`course.${courseSlug}.overview.cards`, cards);
  const { items: dynBadPoints, addItem: addBadPoint, removeItem: removeBadPoint } = useDynamicCmsList(`course.${courseSlug}.overview.badPoints`, badPoints);
  const { items: dynGoodPoints, addItem: addGoodPoint, removeItem: removeGoodPoint } = useDynamicCmsList(`course.${courseSlug}.overview.goodPoints`, goodPoints);
...
function TabCurriculum({ courseSlug }: { courseSlug: string }) {
  ...
  const { items: modules } = useDynamicCmsList(`course.${courseSlug}.curriculum.modules`, defaultModules);
...
function TabWhatsIncluded({ courseSlug }: { courseSlug: string }) {
  ...
  const { items: features } = useDynamicCmsList(`course.${courseSlug}.included.features`, defaultFeatures);

  return (
    <div className="space-y-8 font-bangla animate-in fade-in">
      <h2 className="text-2xl font-bold text-center">মাস্টারক্লাসে আপনি যা যা পাচ্ছেন</h2>
      <div className="grid md:grid-cols-3 gap-6">
        {features?.map((f, i) => (
...
function TabHowItWorks({ courseSlug }: { courseSlug: string }) {
  ...
  const { items: steps } = useDynamicCmsList(`course.${courseSlug}.howitworks.steps`, defaultSteps);

  return (
    <div className="space-y-8 font-bangla animate-in fade-in">
      <h2 className="text-2xl font-bold text-center">কীভাবে ব্যাচে যুক্ত হবেন?</h2>
      <div className="grid md:grid-cols-3 gap-6">
        {steps?.map((s, i) => (
...
function TabFaq({ courseSlug }: { courseSlug: string }) {
  ...
  const { items: faqs } = useDynamicCmsList(`course.${courseSlug}.faq.items`, defaultFaqs);
...
function BatchRegularLanding({ course, slug, initialEnroll }: { course: any, slug: string, initialEnroll: boolean }) {
  const [showModal, setShowModal] = useState<boolean>(initialEnroll);
            ...
            <div className="glass-strong rounded-3xl border border-border/70 shadow-lg relative overflow-hidden backdrop-blur-md">
              <div className="block lg:hidden w-full border-b border-border/40 select-none">
                <div onClick={() => previewVideoId && setActiveVideo(previewVideoId)} onContextMenu={(e) => e.preventDefault()} className={`relative aspect-video w-full group select-none ${previewVideoId ? "cursor-pointer" : ""}`}>
                  <img src={course.thumb || ""} alt={course.title} className="w-full h-full object-cover pointer-events-none select-none" />
                  {previewVideoId && (
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-28">
            <div className="glass-strong rounded-3xl p-6 sm:p-7 border border-border/60 shadow-xl overflow-hidden backdrop-blur-md select-none">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2.5 mb-4 pb-1">
                ...
                <span className="px-3 py-1 text-xs font-semibold rounded-full bg-primary/10 text-primary border border-primary/20 font-sans tracking-wide">40% OFF</span>
              </div>
...
function CourseDetailMaster() {
  ...
  const [enrollParam, setEnrollParam] = useState(false);

  // URL থেকে প্যারামিটার ধরা (নিরাপদ পদ্ধতি)
  useEffect(() => {
  ...
  if (!course) {
    return (
      <SiteShell>
        <div className="p-20 text-center font-bold text-2xl text-destructive mt-20">Course not found.</div>
      </SiteShell>
    );
  }
    ...
    <div className="[&>div>footer]:!hidden [&>footer]:!hidden">
      ...
      <SiteShell>
        ...
        {isBatch1 ? (
          ...
          <BatchRegularLanding course={course} slug={slug} initialEnroll={enrollParam} />
        )}
